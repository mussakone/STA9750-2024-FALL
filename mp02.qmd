...
# Check the structure of the data frame to find the correct year column
str(success_by_genre_period)

# Let's assume the year column is named `release_year`
# Define the genres you want to filter for
selected_genres <- c("Action", "Drama", "Comedy", "Horror", "Sci-Fi")

# Filter for the selected genres
genre_success_popularity_selected <- success_by_genre_period |>
    filter(genres %in% selected_genres)

# Plot: Success trends for selected genres over time
ggplot(genre_success_popularity_selected, aes(x = factor(decade), y = num_successes, color = genres, group = genres)) +
    geom_line(size = 1.5) +
    geom_point(size = 3) +
    labs(title = "Success Trends for Selected Genres Over Time",
         x = "Decade", y = "Number of Successes",
         color = "Genre") +
    theme_minimal()
...
pitch <- "
<b>Title:</b> <i>Shadows in the Horizon</i><br>
<br>
<b>Plot Overview:</b> In a dystopian future where artificial intelligence has gained near-total control over human society, a group of unlikely heroes comes together to dismantle the system that has suppressed freedom for decades. Their journey takes them through moral dilemmas, high-stakes battles, and the ultimate realization that the enemy may not be as simple as it seems.<br>
<br>
<b>Why This Team?</b><br>
<b>Michael B. Jordan:</b> Known for his incredible performances in films like <i>Creed</i> and <i>Black Panther</i>, Michael B. Jordan brings intensity and emotional depth to the lead role. His ability to embody both strength and vulnerability makes him the perfect choice to lead a rebellion against an all-powerful system.<br>
<b>Scarlett Johansson:</b> With her iconic performances in <i>Lucy</i> and the <i>Avengers</i> series, Scarlett Johansson brings both charisma and complexity to the role of a former AI scientist who questions her past decisions. Her experience in action-packed, sci-fi roles makes her an invaluable part of the cast.<br>
<b>Denis Villeneuve:</b> As the visionary director behind <i>Blade Runner 2049</i> and <i>Dune</i>, Denis Villeneuve is the ideal choice to bring this dark, futuristic world to life. His exceptional ability to combine stunning visuals with intricate storytelling will elevate <i>Shadows in the Horizon</i> beyond a typical sci-fi film.<br>
<br>
<b>Supporting Data:</b><br>
We have identified key personnel whose past projects in the action/sci-fi genres have been immensely successful, as shown by both IMDb ratings and our custom success metric. Below is a graphical representation of their success:<br>"
```
```{r}
# Load necessary libraries
library(ggplot2)
